...
def volume_conversions(convert_from_to):
    if convert_from_to in volume_conversions_dict.keys():
        return volume_conversions_dict[convert_from_to]
        ...
        if split_converts[0] == split_converts[1]:
            ...
            if reversed_conversion in list(volume_conversions_dict.keys()):
                result = volume_conversions_dict[reversed_conversion]
                result = 1 / result
                return result

            else:
                first_change_str = volume_conversions_dict["base"] + '-' + split_converts[0]
                first_change_result = volume_conversions_dict[first_change_str]
                first_change_result = 1 / first_change_result

                second_change_str = volume_conversions_dict["base"] + '-' + split_converts[1]
                second_change_result = volume_conversions_dict[second_change_str]

                return first_change_result * second_change_result


def convert_volume(from_unit, to_unit):
    convert_from_to = from_unit + '-' + to_unit
    return volume_conversions(convert_from_to)
